...
| R05 | 物流方向与等待时限 | 退货物流轨迹中出现“长沙”字段 | 未出现“长沙”且距首条“已揽收/已收件”轨迹 `<72小时`时进入等待复查；`≥72小时`仍未出现“长沙”时高风险转人工 |
| R06 | 物流更新时间 | 当前时间减去最新物流节点时间 `<72小时` | `≥72小时`高风险转人工 |
| R07 | 平台处理倒计时 | 剩余处理时间 `≥72小时` | `<72小时`高风险转人工 |
| R08 | 必要字段 | 售后编号、订单号、店铺、退款金额、退货件数、退货物流单号、物流轨迹、最新物流时间、处理倒计时均可可靠读取；未出现“长沙”时还须可靠读取首条“已揽收/已收件”时间 | 中风险转人工 |
...
- 物流未出现“长沙”时，以首条“已揽收/已收件”有效轨迹时间为起点；不足 `72小时`时不转人工，任务进入等待复查并由每 30 分钟扫描重新判断；达到或超过 `72小时`仍未出现“长沙”时立即转人工。
- 不读取“已送达”“待取件”“已签收”“代收”等关键词来判断是否送达其他地点；这些文案不单独触发转人工。
- 本场景不访问 OMS，仓库信息不参与自动退款判断、转人工通知或看板健康检查。
- “首条已揽收/已收件轨迹”取平台当前展示的完整有效物流轨迹中，时间最早且状态语义为“已揽收”或“已收件”的节点；无法可靠识别该节点时按必要字段识别失败转人工。
...
4. 金额、件数、物流方向、首条揽收后 72 小时、最新物流 72 小时、处理倒计时规则：等待复查、转人工或放行。
5. 全部条件满足：执行自动退款。
...
flowchart TD
    ...
    J --> L{字段和页面是否正常}
    L -->|否| M[生成中风险人工工单]
    L -->|是| N{是否命中金额、件数、最新物流或倒计时等转人工条件}
    N -->|是| P[生成高风险人工工单]
    N -->|否| O{物流轨迹是否出现长沙}
    O -->|是| Q[持锁重读全部输入并重算R01-R09]
    O -->|否| OA{距首条已揽收/已收件是否达到72小时}
    OA -->|是| P
    OA -->|否| OB[进入等待复查]
    OB --> OC[下一个30分钟扫描周期重新判断]
    Q --> R{全部条件仍满足}
...
**异常分支：** 物流轨迹未显示“长沙”时，以首条“已揽收/已收件”轨迹时间起算：不足 72 小时不执行退款、不转人工，等待下一次 30 分钟扫描复查；达到或超过 72 小时仍未显示“长沙”时转人工。命中金额、件数、最新物流节点停滞或平台剩余时间等其他转人工条件时立即转人工。当前流程不点击“拒绝退款”。
...
│   ├── 金额与件数判断
│   ├── 长沙物流方向判断
│   ├── 首条揽收未发往长沙、最新物流停滞、平台剩余时间三类72小时判断
│   └── 标准转人工原因码
├── P0 自动退款执行
│   ├── 执行前状态复核
│   ├── 确认退款
...
- 物流已出现“长沙”时，无需再计算首条揽收时间；未出现“长沙”时必须识别首条“已揽收/已收件”时间并计算经过时长。
...
2. 任一条件不再满足时，禁止点击退款，并按最新条件进入等待复查、跳过或转人工。
...
| first_collected_time | datetime | 条件必填 | PDD/规则引擎 | 未出现“长沙”时必填；完整轨迹中首条“已揽收/已收件”有效节点时间 |
| hours_since_first_collected | decimal | 条件必填 | 系统计算 | 未出现“长沙”时必填；当前时间减首条“已揽收/已收件”时间 |
...
| waiting_recheck | 未出现“长沙”且距首条“已揽收/已收件”不足72小时 | 等待复查 | 下一个30分钟扫描周期重新读取物流并判断，不生成转人工工单 |
...
| 高 | LOGISTICS_NOT_TO_CHANGSHA_72H | 退货物流72小时未发往长沙 | 从首条“已揽收/已收件”轨迹起已 `≥72小时`，物流轨迹仍未出现“长沙” |
...
| 配置项 | 已确认值 | 实现要求 |
|---|---|---|
| 通知群 | 拼多多工单Agent对接群 | 使用拼多多专属对接群配置 |
| 钉钉群号 | `193120000457` | 作为受控配置项，不在业务逻辑中散落硬编码 |
| 成功订单 | 自动处理成功不通知 | 不发送成功消息，不触发 @ |
| 转人工/系统异常 | 发送通知 | 使用下述固定标题、模板和 @ 规则 |
| @ 方式 | 固定逐一 @9 位指定人员 | 不使用“@所有人”，群内其他成员不 @ |

**固定 @ 人员**：狄云、青顾、行云、今夏、婉安、子宁、霜寒、听竹、梨花。

人员显示名称为业务确认口径；开发应使用业务提供的受控成员映射配置对应用户标识，手机号等个人标识不得写入 PRD、普通配置或日志。人员调整属于通知配置变更，不要求修改业务判断代码，但必须记录变更前后值和确认人。
...
- `风险等级`显示“高”或“中”；
- 自动退款明确成功时不发送钉钉消息；
- 转人工和系统异常通知必须发送至“拼多多工单Agent对接群”（群号 `193120000457`），逐一 @ 已确认的 9 位人员，不得 @所有人或额外 @ 群内其他成员；
...
| 未出现“长沙”且距首条“已揽收/已收件”不足72小时 | 不退款、不生成转人工工单 | 进入等待复查，下一个30分钟扫描周期重新判断 |
| 未出现“长沙”且无法识别首条“已揽收/已收件”时间 | 禁止使用售后创建时间或上传物流单号时间替代 | 中风险转人工 |
...
- 同一售后单的钉钉转人工消息不得重复发送。
- 自动退款明确成功的任务不得发送钉钉通知。
- 转人工和系统异常通知必须发往已确认群，并仅逐一 @ 已确认的 9 位人员。
- 退款结果未知时不得自动重试不可逆操作。
...
| 用例 | 退款金额 | 退货件数 | 物流含长沙 | 距首条揽收/收件 | 距最新物流 | 剩余时间 | 期望结果 |
|---|---:|---:|---|---|---|---|---|
| A01 | 499.99 | 4 | 是 | — | 71小时59分 | 72小时 | 自动退款 |
| A02 | 500.00 | 4 | 是 | — | 10小时 | 100小时 | 高风险转人工：退款金额超限 |
| A03 | 100.00 | 5 | 是 | — | 10小时 | 100小时 | 高风险转人工：退货件数超限 |
| A04 | 100.00 | 1 | 否 | 71小时59分 | 10小时 | 100小时 | 等待复查，不退款、不转人工 |
| A05 | 100.00 | 1 | 否 | 72小时 | 10小时 | 100小时 | 高风险转人工：退货物流72小时未发往长沙 |
| A06 | 100.00 | 1 | 是 | — | 72小时 | 100小时 | 高风险转人工：物流72小时未更新 |
| A07 | 100.00 | 1 | 是 | — | 10小时 | 71小时59分 | 高风险转人工：临近超时 |
| A08 | 500.00 | 5 | 否 | 80小时 | 80小时 | 20小时 | 只生成一条高风险消息，列出全部原因 |
| A09 | 缺失 | 1 | 是 | — | 10小时 | 100小时 | 中风险转人工：退款信息缺失 |
| A10 | 100.00 | 1 | 是 | — | 10小时 | 100小时 | 平台明确成功，记录自动退款成功 |
| A11 | 100.00 | 0 | 是 | — | 10小时 | 100小时 | 中风险转人工：退货件数非法 |
| A12 | 100.00 | 1 | 是 | — | 10小时 | 100小时 | 售后类型非退货退款时直接跳过 |
| A13 | 0.00 | 1 | 是 | — | 10小时 | 100小时 | 中风险转人工：退款金额非法 |
| A14 | 100.00 | 1 | 否 | 无法识别 | 10小时 | 100小时 | 中风险转人工：首条揽收/收件时间无法识别 |
...
| B13 | 首次判断后跨过金额、件数或任一72小时边界 | 点击前重算R01-R09并按最新结果等待复查、停止或转人工 |
...
| B21 | 转人工或系统异常且钉钉可用 | 仅向“拼多多工单Agent对接群”（群号 `193120000457`）发送一条通知，逐一 @ 固定 9 人，不使用“@所有人”且不 @ 其他群成员 |
...
1. 本文 P0 功能全部完成并通过测试；
2. A01-A14、B01-B21 验收用例全部通过；
3. 三家店铺 ID、退款权限和场景开关配置完成；
4. PDD、钉钉和现有看板联调通过；
5. 唯一键、执行前复核、结果未知禁止重试和紧急暂停能力可用；
6. 【本次更新】钉钉群名称、群号、成功不通知和固定 @9 人配置验证通过；
7. 真实退款操作日志和证据截图可查询；
...
- 钉钉消息幂等与发送失败告警机制；
- 已确认“拼多多工单Agent对接群”群号 `193120000457`、成功不通知及固定 @9 人映射的上线前技术验证；
